...
app.get("/", (request, response) => {
  console.log(Date.now() + "ukqzn | YouTube");
...
fs.readdir('./komutlar/', (err, files) => {
    ...
    files.forEach(f => {
        ...
        props.conf.aliases.forEach(alias => {
            client.aliases.set(alias, props.help.name);//batinay
        });
    });
});
...
client.reload = command => {
    return new Promise((resolve, reject) => {
        try {
            delete require.cache[require.resolve(`./komutlar/${command}`)];//batinay
            let cmd = require(`./komutlar/${command}`);
            client.commands.delete(command);
            client.aliases.forEach((cmd, alias) => {
                if (cmd === command) client.aliases.delete(alias);//batinay
            });
...
client.load = command => {
    return new Promise((resolve, reject) => {
        try {
            let cmd = require(`./komutlar/${command}`);
            client.commands.set(command, cmd);
            cmd.conf.aliases.forEach(alias => {//batinay
                client.aliases.set(alias, cmd.help.name);
            });
            resolve();
        } catch (e) {
